...
class WhatsAppService {
  ...
  async sendWelcome(phone) {
    ...
    const phoneNumberId = config.phone_number_id || process.env.PHONE_NUMBER_ID;
    const accessToken = config.whatsapp_access_token || process.env.WHATSAPP_ACCESS_TOKEN;

    // --- Gujarati Welcome Message (from DB or fallback) ---
    const guMsg = config.welcome_message_gu || `🏡 સોમેશ્વર ગાર્ડન સિટી, મહેસાણા માં આપનું હાર્દિક સ્વાગત છે!
...
✨ આપના સપનાનું ઘર હવે મહેસાણામાં. આજે જ મુલાકાત લો!`;
    ...
    // --- Hindi Welcome Message (from DB or fallback) ---
    const hiMsg = config.welcome_message_hi || `सोमेश्वर गार्डन सिटी, मेहसाणा में आपका हार्दिक स्वागत है! 🙏
...
✨ अपने सपनों का घर आज ही बुक करें।`;
    ...
    // --- Location Message (from DB or fallback) ---
    const locMsg = config.location_message || `📍 Prime Location – Someshwar Garden City
...
async sendBrochure(phone) {
    ...
    const phoneNumberId =
        config.phone_number_id || process.env.PHONE_NUMBER_ID;
  ...
  async sendCustomeMessage(phone) {
    ...
    const customMsg = config.custom_message || "આ AI જનરેટેડ મેસેજ છે, વધુ માહિતી માટે +91 98250 15196 પર સંપર્ક કરો.";
